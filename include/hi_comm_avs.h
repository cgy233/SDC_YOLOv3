...
{
    AVS_LUT_ACCURACY_E  enAccuracy;               /* RW; LUT accuracy. */
...
{
    AVS_GAIN_MODE_E enMode;                  /* RW; Gain mode. */
...
typedef struct hiAVS_GRP_ATTR_S
{
    ...
    HI_U32               u32PipeNum;                    /* RW; Range: Hi3559AV100ES = [1, 6] | Hi3559AV100 = [1, 8] | Hi3519AV100 = [1, 4]; Pipe number. */
    HI_BOOL              bSyncPipe;                     /* RW; Whether sync pipe image. */

    AVS_LUT_S            stLUT;                         /* Look up table. */
    AVS_GAIN_ATTR_S      stGainAttr;                    /* Gain attribute. */
    HI_U64               u64BBoxPhyAddr[AVS_PIPE_NUM];  /* Physical address of bounding box data. */
...
{
    HI_U32              u32Width;           /* RW; Range: Hi3559AV100ES = [256, 16384] | Hi3559AV100 = [256, 16384] | Hi3519AV100 = [256, 8192]; Width of target image. */
...
{
    HI_U32 u32WorkingSetSize;     /* Working set size. */
}AVS_MOD_PARAM_S;
